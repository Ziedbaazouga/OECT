classdef OECT_GUI < matlab.apps.AppBase
    ...
    methods (Access = public)
        ...
        function buildUI(app)
            ...
            % One row per left-hand panel (Model, Data, Geometry, Params, Fit).
            % Params gets '1x' so it absorbs/shrinks with available space while
            % the other panels (including Fit) always size to their content, so
            % the Fit panel never gets pushed off-screen; LeftPanel is
            % Scrollable so it remains reachable even on small windows.
            app.LeftGrid = uigridlayout(app.LeftPanel, [5, 1], ...
                'RowHeight', {'fit', 'fit', 'fit', '1x', 'fit'}, ...
                'BackgroundColor', [0.15, 0.15, 0.15]);
