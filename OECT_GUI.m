...
drawnow;  % Force GUI to update
    ...
    app.updateUIState();
        ...
        function cancelFit(app)
            if app.isFitting
                ...
                if ~isempty(app.model) && isobject(app.model)
                    app.model.setStopFlag(true);
                end
            end
        end
    ...
    methods (Access = public)
        ...
        function buildUI(app)
            ...
            app.LiveTuneBtn = uibutton(app.ExportGrid, 'Text', 'Live Tune', 'BackgroundColor', [0.3, 0.3, 0.4], 'FontColor', [1, 1, 1], 'ButtonPushedFcn', @(src, evt) app.LiveTuneBtnPushed());
            
            % Status Bar
            app.StatusBar = uilabel(app.RightGrid, 'Text', 'Ready', 'FontColor', [0.8, 0.8, 0.8]);
        end
    end
end